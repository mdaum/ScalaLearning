 ...
 */
 class ListsSuite {
  ...
  @Test def `sum of more numbers (10pts)`: Unit = {
    assertEquals(sum(List(-4,4,1)), 1)
  }
  ...
  @Test def `max of a few more numbers (10pts)`: Unit = {
    assertEquals(max(List(Integer.MAX_VALUE, -100000, 50000)), Integer.MAX_VALUE)
  }
